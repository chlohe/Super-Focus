...
class T4PlaneExtrapolationDataset(Dataset):

    def __init__(self, embryo_dir_globs=['PLACEHOLDER', 'PLACEHOLDER'],
                 plane_size=(400, 400), use_normalisation=True, use_augmentations=False, direction='up', verbose=False):
    ...
    def __getitem__(self, idx):
        ...
        try:
            if self.direction == 'up':
                label_idx = random.randint(7, len(plane_imgs) - 1)
            else:
            ...
            if self.direction == 'up':
                image[:, :, 0] = plane_imgs[label_idx - 1][1] 
                image[:, :, 1] = plane_imgs[label_idx - 2][1]
            else:
                image[:, :, 0] = plane_imgs[label_idx + 1][1] 
                image[:, :, 1] = plane_imgs[label_idx + 2][1]
